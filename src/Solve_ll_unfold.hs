...
solve_ll_unfold :: Matrix -> Either T.Text [Double]
solve_ll_unfold mat = checkMatrix mat >>= calcTriangle >>= backInsert
...
calcTriangle :: Matrix -> Either T.Text [Equation]
calcTriangle = unfoldM pivotStep
  where
    pivotStep :: Matrix -> Either T.Text (Maybe (Equation, Matrix))
    pivotStep []  = Right Nothing
    pivotStep mat0 =
        if abs negPivot < cLIMIT
        then Left cNONSOLVABLE
        else Right (Just (pivotRow, map newRow newMat))

          -- Right (Just (pivotrow, map newRow newMat))
      where
